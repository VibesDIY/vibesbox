...
// The complete iframe.html content as a string constant
    ...
    <script>
      ...
      function executeCode(data) {
        try {
          ...
          const transformedCode = window.Babel.transform(data.code, {
            presets: ['react'],
            plugins: ['transform-modules-umd'],
          });

          // Create a module-like environment and execute the code
          const moduleCode = \`
            (function() {
          ...
          const AppComponent = eval(moduleCode);
          
          // Import React DOM client for rendering
          import('react-dom/client').then((ReactDOMClient) => {
            if (!window.babelTransformError && AppComponent) {
              const root = ReactDOMClient.createRoot(container);
              
              // Import React to create the element
            ...
            } else if (window.babelTransformError) {
